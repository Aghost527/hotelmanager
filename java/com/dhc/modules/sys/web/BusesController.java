...
import com.dhc.modules.sys.entity.Buses;
import com.dhc.modules.sys.service.BusesManager;
...
@Controller
@RequestMapping(value = "${adminPath}/sys/buses")
public class BusesController extends BaseController<Buses, Long> {

	@Autowired
	private UserManager userManager;

	public final static String SSSION_SEARCH = "Buses_SEARCH";
	...
	@Override
	public EntityManager<Buses, Long> getEntityManager() {
		return busesManager;
	}

	@RequestMapping(value = { "" })
	public String list() {
		return "modules/sys/buses";
	}

	@RequestMapping(value = { "input" })
	public String input() {
		return "modules/sys/buses-input";
	}
	...
	@ResponseBody
	public Result save(@ModelAttribute("model") Buses model) {
		Result result;
		// 名称重复校验
//		Buses checkBuses = busesManager.findUniqueBy("title",
//				model.getTitle());
//		if (checkBuses != null
//				&& !checkBuses.getId().equals(model.getId())) {
//			result = new Result(Result.WARN, "标题 [" + model.getTitle()
//					+ "]已存在,请写一个不同的标题", "title");
//			return result;
//		}
		busesManager.saveEntity(model);
	...
	public Datagrid datagrid() {
		...
		Page<Buses> p = new Page<Buses>(SpringMVCHolder.getRequest());
		p = getEntityManager().findPage(p, filters);

		Datagrid<Buses> dg = new Datagrid<Buses>(p.getTotalCount(),
				p.getResult());
		return dg;
	}
}
