...
import com.dhc.modules.sys.service.CustomersManager;
...
public class CustomersController extends BaseController<Customers, Long> {

	@Autowired
	private UserManager userManager;

	public final static String SSSION_SEARCH = "Customers_SEARCH";
	...
	@Override
	public EntityManager<Customers, Long> getEntityManager() {
		return customersManager;
	}

	@RequestMapping(value = { "" })
	public String list() {
		return "modules/sys/customers";
	}

	@RequestMapping(value = { "input" })
	public String input() {
		return "modules/sys/customers-input";
	}
	...
	@ResponseBody
	public Result save(@ModelAttribute("model") Customers model) {
		Result result;
		// 名称重复校验
//		Customers checkCustomers = customersManager.findUniqueBy("title",
//				model.getTitle());
//		if (checkCustomers != null
//				&& !checkCustomers.getId().equals(model.getId())) {
//			result = new Result(Result.WARN, "标题 [" + model.getTitle()
//					+ "]已存在,请写一个不同的标题", "title");
//			return result;
//		}
		customersManager.saveEntity(model);
	...
	public Datagrid datagrid() {
		...
		Page<Customers> p = new Page<Customers>(SpringMVCHolder.getRequest());
		p = getEntityManager().findPage(p, filters);

		Datagrid<Customers> dg = new Datagrid<Customers>(p.getTotalCount(),
				p.getResult());
		return dg;
	}
}
