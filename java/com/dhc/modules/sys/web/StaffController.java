...
import com.dhc.modules.sys.entity.Staff;
import com.dhc.modules.sys.service.StaffManager;
...
@Controller
@RequestMapping(value = "${adminPath}/sys/staff")
public class StaffController extends BaseController<Staff, Long> {

	@Autowired
	private UserManager userManager;

	public final static String SSSION_SEARCH = "Staff_SEARCH";
	...
	@Override
	public EntityManager<Staff, Long> getEntityManager() {
		return staffManager;
	}

	@RequestMapping(value = { "" })
	public String list() {
		return "modules/sys/staff";
	}

	@RequestMapping(value = { "input" })
	public String input() {
		return "modules/sys/staff-input";
	}
	...
	@ResponseBody
	public Result save(@ModelAttribute("model") Staff model) {
		Result result;
		// 名称重复校验
//		Staff checkStaff = staffManager.findUniqueBy("title",
//				model.getTitle());
//		if (checkStaff != null
//				&& !checkStaff.getId().equals(model.getId())) {
//			result = new Result(Result.WARN, "标题 [" + model.getTitle()
//					+ "]已存在,请写一个不同的标题", "title");
//			return result;
//		}
		staffManager.saveEntity(model);
	...
	public Datagrid datagrid() {
		...
		Page<Staff> p = new Page<Staff>(SpringMVCHolder.getRequest());
		p = getEntityManager().findPage(p, filters);

		Datagrid<Staff> dg = new Datagrid<Staff>(p.getTotalCount(),
				p.getResult());
		return dg;
	}
}
